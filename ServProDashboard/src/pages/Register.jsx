...
const Register = () => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-10 sm:px-6 lg:px-8">
      <div className="relative w-full max-w-2xl overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white/90 p-6 shadow-2xl shadow-slate-900/10 backdrop-blur sm:p-8">
        <div className="absolute -left-12 -top-12 h-44 w-44 rounded-full bg-emerald-200/45 blur-3xl" />
        <div className="relative z-10">
          ...
          <form className="grid gap-4 sm:grid-cols-2" onSubmit={handleSubmit}>
            ...
            {formData.type === 'PROVIDER' && (
              <>
                ...
                <div className="space-y-2">
                  <label htmlFor="address" className="text-sm font-semibold text-slate-700">{t('auth.address')}</label>
                  <input
                    type="text"
                    id="address"
                    name="address"
            ...
            <button
              type="submit"
              className="sm:col-span-2 mt-2 w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
              disabled={loading}
            >
          ...
          <div className="mt-6 text-center text-sm text-slate-600">
            <p>
              {t('auth.haveAccount')}{' '}
              <Link to="/login" className="font-bold text-sky-700 hover:text-sky-600">
                {t('auth.loginLink')}
              </Link>
            </p>
          </div>
